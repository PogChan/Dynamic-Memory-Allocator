...
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	...
	double x = sf_internal_fragmentation();
	cr_assert_float_eq(x, 0.520833, 0.001,"should be within the range 0.520833 to 0.521, ycou got %lf", sf_internal_fragmentation());
}

Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT) {
	cr_assert_float_eq(sf_peak_utilization(), 0.0, 0.0000000000001, "HEAP IS EMPTY NO UTILIZATION");

}

Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT) {
	sf_malloc(3000);
	double peak = sf_peak_utilization();
	cr_assert_float_eq(peak, 0.976562, 0.001, "Expected %f but got %f", 0.976562,peak );
...
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT) { 
	...
	char* a = sf_malloc(sizeof(char));
	char* b = sf_malloc(sizeof(char));
	char* c = sf_malloc(sizeof(char));
	char* d = sf_malloc(sizeof(char));
	char* e = sf_malloc(sizeof(char));
	char* f = sf_malloc(sizeof(char));
	sf_free(a);
	sf_free(b);
	sf_free(c);
	sf_free(d);
...
Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT) {
        int *smallInt = sf_malloc(sizeof(int));
        *smallInt = 4;
        int *bigInt = sf_realloc(smallInt, sizeof(int)*2);

        cr_assert_eq(smallInt, bigInt, "YOu are not supposed to malloc this one as we got enough block size/padding to fit the request");
}
